Public Class TamanTemaWonderlandOnlineTiketdanTempahan
    ...
    Private Sub TamanTemaWonderlandOnlineTiketdanTempahan_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        lblWTPOTB.Text = " Taman Tema Wonderland Online Tiket dan Tempahan"
        gpCustomerDetails.Text = "Maklumat Pelanggan "
        lblThemePark.Text = "Taman Tema :"
        lblCustomerName.Text = "Name Pelanggan :"
        lblCustomerNumber.Text = "Nombor Pelanggan :"
        lblAdults.Text = "Dewasa :"
        lblSeniorCitizen.Text = "Warga Emas :"
        lblChildren.Text = "Kanak-Kanak :"

        gpTicketTypes.Text = "Jenis Tiket "
        rbRegular.Text = "Biasa"
        rbExpress.Text = "Laju"
        rbExpressUnlimited.Text = "Laju yang tidak terhad"
        rbVVIP.Text = "VVIP"

        gpCosts.Text = "Harga-harga"
        lblTickets.Text = "Tikets :"
        lblTypes.Text = "Jenis-jenis :"
        lblBoatandTrainRide.Text = "Perjalanan Kapal & Keretapi :"
        lblShows.Text = "Tontonan ;"
        lblFoodMenu.Text = "Menu Hidangan ;"
        lblSubTotal1.Text = "Jumlah Bahagian(1) :"
        lblDiscount.Text = "Diskaun :"
        lblSubTotal2.Text = "Jumlah Bahagian(2) :"
        lblGST.Text = "GST 6% :"
        lblTotalPackage.Text = "Jumlah Paket :"

        gpBoatandTrainRide.Text = "Perjalanan Kapal & Keretapi"
        cbCruiseRide.Text = "Pelayaran"
        cbBananaBoatRide.Text = "Perahu Pisang"
        cbDuckBoatRide.Text = "Perahu Itik"
        cbLocomotiveRide.Text = "Kepala Kereta Api"
        cbElectricTrainRide.Text = "Kereta Api Elektrik"
        cbCoupleTrainRide.Text = "Kereta Api Pasangan"

        gpShows.Text = "Tontonan"
        cbMagicalShow.Text = "Magik"
        cbDolphinShow.Text = "Ikan Dolfin"
        cbDayFireWorkShow.Text = "Bunga Api Siang"
        cbNightFireworkShow.Text = "Bunga Api Malam"

        gpFoodMenu.Text = "Menu Hidangan"
        cbWesternMenu.Text = "Menu Kebaratan"
        cbChineseMenu.Text = "Menu Ketimuran"
        cbMalayMenu.Text = "Menu Melayu"
        cbJapaneseMenu.Text = "Menu Jepun"

        lblRefNo.Text = "Nombor Rujukan :"
        lblQuoteDate.Text = "Tarikh Kutipan :"
        lblValid.Text = "Sah Hingga :"

        btnCalculateQuote.Text = "Mengira Sebut Harga"
        btnClearAll.Text = "Membatalkan Semua"
        btnBookThePackage.Text = "Tempahan Pakej"
        btnExit.Text = "Keluar"
    ...
    Private Function CalculateBoatAndTrainRide(ByVal intNumberOfAdults, ByVal intNumberOfSeniorCitizen, ByVal intNumberOfChildren) As Double
        ...
        If intNumberOfAdults < 2 AndAlso cbCoupleTrainRide.Checked = True Then
            MessageBox.Show("Kereta Api Pasangan dibuka kepada orang dewasa sahaja.Pastikan nombor orang dewasa sekurang-kurangnya 2 orang. ", "Perhatian")
        End If
    ...
    Private Function ValidateTicketTypesAvailability(ByVal NumberOfAdults As Integer, ByVal NumberOfSeniorCitizen As Integer, ByVal NumberOfChildren As Integer, ByVal Count As Integer) As Integer

        'intMax = Maximum Availability per booking
        Dim intMax As Integer

        'Total number of customer
        Count = NumberOfAdults + NumberOfSeniorCitizen + NumberOfChildren
        ...
        ElseIf rbVVIP.Checked Then
            intMax = 5
        End If
        If Count > intMax Then 'if total number of customer > maximum availability per booking
            Return Count = False 'ticket type cannot accommodate to the number of customer
        Else
            Return Count
        End If
    ...
    Private Sub btnCalculateQuote_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnCalculateQuote.Click
        ...
        Dim intNumberOfAdults, intNoAdults, intNumberOfSeniorCitizen, intNoSeniorCitizen, intNumberOfChildren, intNoChildren, intNoPeople, intCount As Integer
        ...
        intCount = ValidateTicketTypesAvailability(intNumberOfAdults, intNumberOfSeniorCitizen, intNumberOfChildren, intCount)
        ...
        If strCustomerName = "" Then
            strErrorMessage += "Nama Pelanggan Tidak Sah" + ControlChars.NewLine
            blnChecker = False ' input data fail the validation test
        End If
       
        If strCustomerNumber = "" Then
            strErrorMessage += "Nombor Telephone Tidak Sah" + ControlChars.NewLine
            blnChecker = False ' input data fail the validation test
        End If
        

        If intNumberOfAdults = -1 Then
            strErrorMessage += "Bilangan Dewasa Tidak Sah" + ControlChars.NewLine
            blnChecker = False ' input data fail the validation test
        End If
        If intNumberOfSeniorCitizen = -1 Then
            strErrorMessage += "Bilangan Orang Tua Tidak Sah" + ControlChars.NewLine
            blnChecker = False ' input data fail the validation test
        End If
        If intNumberOfChildren = -1 Then
            strErrorMessage += "Bilangan Kanak-Kanak Tidak Sah" + ControlChars.NewLine
            blnChecker = False ' input data fail the validation test
        End If

        If blnTicketTypesChecker = False Then 'Ticket types radio button is unchecked
            strErrorMessage += "Jenis Tiket Tidak Sah." & " Pastikan Pilihan Tiket Sudah Dipilih" + ControlChars.NewLine
            blnChecker = False
        End If

        If blnChecker = False Then ' input data fail the validation test
            MessageBox.Show(strErrorMessage, "Kesilapan Mesej")
            Exit Sub
        End If

        If intCount = False Then 'number of people exceed maximum availability per booking
            MessageBox.Show("Pilihan Tiket Melebihi Maksimum Ketersediaan. Sila buat tempahan tiket berasing-asing", "Kesilapan Mesej")
            Exit Sub
        End If
    ...
    Private Sub btnBookThePackage_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnBookThePackage.Click
        ...
        If lblAnsTotalPackage.Text = "" Then
            MessageBox.Show("Tempahan Tiket Taman Tema Wonderland gagal. Sila buat tempahan tiket sekali lagi!", " Perhatian", MessageBoxButtons.OK, MessageBoxIcon.Error)
        Else
            ...
            MessageBox.Show("Tempahan Tiket Taman Tema Wonderland Sudah Berjaya !", "Taman Tema Wonderland", MessageBoxButtons.OK, MessageBoxIcon.Exclamation, MessageBoxDefaultButton.Button1)
        End If
    End Sub
    ...
    Private Sub Mainform_Closing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        ...
        dlgButton = MessageBox.Show(" Pastikan Anda Mahu Keluar?", "Taman Tema Wonderland", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation, MessageBoxDefaultButton.Button1)
